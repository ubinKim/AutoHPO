...
if __name__ == "__main__":
    ...
    # Define the number of points on the search space for both grid and random search.
    num_points = len(grid_lr) * len(grid_drop) * len(grid_hidden)
    count = 0

    # Hyperparameter optimization using grid and random search
    with open('../results/grid_results.csv', 'w') as grid_results_file, open('../results/random_results.csv', 'w') as random_results_file:
        grid_results_file.write(header)
        random_results_file.write(header)

        # tqdm: print progress bar
        for lr, dropout, hidden_layer in tqdm(list(itertools.product(grid_lr, grid_drop, grid_hidden)), total=num_points):
            count += 1
